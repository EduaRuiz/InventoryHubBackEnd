...
import { ProductBranchIdValueObject } from '.';

describe('ProductBranchIdValueObject', () => {
  ...
  describe('validateData', () => {
    it('should not set error if value is a valid UUID v4', () => {
      ...
      const productBranchId = new ProductBranchIdValueObject(validUUID);
      const expected = false;

      // Act
      productBranchId.validateData();

      // Assert
      expect(productBranchId.hasErrors()).toBe(expected);
    ...
    it('should set error if id is not a valid UUID', () => {
      ...
      const productBranchId = new ProductBranchIdValueObject(invalidUUID);
      const expected = true;
      ...
      // Assert
      expect(productBranchId.hasErrors()).toBe(expected);
      expect(productBranchId.getErrors()[0]?.field).toBe(expectedField);
      expect(productBranchId.getErrors()[0]?.message).toBe(expectedMessage);
