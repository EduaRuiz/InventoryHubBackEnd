import { SellerSaleRegisterUseCase } from './seller-sale-register.use-case';
...
describe('SellerSaleRegisterUseCase', () => {
  let sellerSaleUseCase: SellerSaleRegisterUseCase;
  ...
  beforeEach(() => {
    ...
    sellerSaleUseCase = new SellerSaleRegisterUseCase(
      eventServiceMock,
      eventPublisherMock,
    );
  });
  ...
  describe('execute', () => {
    it('should throw ConflictException if user does not exist or is not registered', (done) => {
      ...
      // Act
      const result = sellerSaleUseCase.execute(command, userId);
    ...
    it('should return a SaleDomainModel if user exists and belongs to the selected branch', (done) => {
      ...
      jest
        .spyOn(sellerSaleUseCase as any, 'sale')
  ...
  describe('sale', () => {
    it('should throw BadRequestException if one of the products does not exist', (done) => {
      ...
      // Act
      const result = (sellerSaleUseCase as any).sale(command, userId);
    ...
    it('should return a SaleDomainModel if all products exist', (done) => {
      ...
      jest
        .spyOn(sellerSaleUseCase as any, 'storeEvents')
        .mockReturnValue(of([eventProductUpdated]));

      jest
        .spyOn(sellerSaleUseCase as any, 'productsInSale')
        .mockReturnValue(of([product]));

      jest
        .spyOn(sellerSaleUseCase as any, 'saleFactory')
        .mockReturnValue(of(sale as SaleDomainModel));

      jest
        .spyOn(sellerSaleUseCase as any, 'saleEventFactory')
        .mockReturnValue(of(saleEvent as EventDomainModel));

      jest
        .spyOn(sellerSaleUseCase as any, 'publishEvents')
      ...
      result.subscribe({
        next: (sale: any) => {
          ...
          expect((sellerSaleUseCase as any).storeEvents).toHaveBeenCalled();
          expect((sellerSaleUseCase as any).productsInSale).toHaveBeenCalled();
          expect((sellerSaleUseCase as any).saleFactory).toHaveBeenCalled();
          expect(
            (sellerSaleUseCase as any).saleEventFactory,
          ).toHaveBeenCalled();
          expect((sellerSaleUseCase as any).publishEvents).toHaveBeenCalled();
  ...
  describe('getProducts', () => {
    it('should throw BadRequestException if one of the products does not exist', (done) => {
      ...
      // Act
      const result = (sellerSaleUseCase as any).getProducts(command);
  ...
  describe('factoryEvents', () => {
    it('should return an array of EventDomainModel', (done) => {
      ...
      // Act
      const result = (sellerSaleUseCase as any).factoryEvents(
        of(products),
      ) as any;
  ...
  describe('storeEvents', () => {
    it('should return an array of EventDomainModel', (done) => {
      ...
      // Act
      const result = (sellerSaleUseCase as any).storeEvents(of(events));
  ...
  describe('publishEvents', () => {
    it('should return an array of EventDomainModel', (done) => {
      ...
      // Act
      const result = (sellerSaleUseCase as any).publishEvents(of(events));
  ...
  describe('productsInSale', () => {
    it('should return an array of ProductDomainModel', (done) => {
      ...
      // Act
      const result = (sellerSaleUseCase as any).productsInSale(
        of(products),
        command,
      ) as any;
  ...
  describe('saleEventFactory', () => {
    it('should return an EventDomainModel', (done) => {
      ...
      const expectedEvent = {
        ...
        eventBody: sale,
        occurredOn: new Date(),
        typeName: TypeNameEnum.SELLER_SALE_REGISTERED,
      };
      jest
        .spyOn(eventServiceMock, 'generateIncrementalSaleId')
        .mockReturnValue(of(1));
      ...
      // Act
      const result = (sellerSaleUseCase as any).saleEventFactory(of(sale));
  ...
  describe('saleFactory', () => {
    it('should return a SaleDomainModel', (done) => {
      ...
      // Act
      const result = (sellerSaleUseCase as any).saleFactory(
        of([productClean]),
